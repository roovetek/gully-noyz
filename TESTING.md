...
- `tab-navigation.spec.ts` - record/timeline/stats/config navigation + admin hash backdoor
- `ball-recording.spec.ts` - manual outcome drawer smoke
...
- `test1-runthrough.spec.ts` - named Test1 runthrough (`2026-04-09-Test1`)
...
- Route: `/#/qa` (hidden; not linked in normal navigation).
- Data source: `public/qa/latest/report.json` generated by `npm run qa:harness`.
- Includes:
...
- `tab-navigation.spec.ts` - Main tab navigation and hidden admin route smoke
- `ball-recording.spec.ts` - Manual recording outcome selection smoke
...
- `test1-runthrough.spec.ts` - Named runthrough for Test1 scenario
